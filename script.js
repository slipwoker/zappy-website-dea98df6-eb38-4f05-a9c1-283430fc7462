document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function getErrorContainer(field) {
      ...
      let errorEl = contactForm.querySelector('[data-error-for="' + id + '"]');

      if (!errorEl) {
        errorEl = document.createElement('span');
        errorEl.setAttribute('data-error-for', id);
    ...
    function getErrorMessage(field, rule) {
      const customMsg = field.getAttribute('data-msg-' + rule);
      ...
      const messages = {
        ...
        minlength: fieldName + ' must be at least ' + field.getAttribute('minlength') + ' characters.'
      };
    ...
    function validateField(field) {
      ...
      if (field.required || field.hasAttribute('required')) {
        if (!VALIDATORS.required(field.type === 'checkbox' ? (field.checked ? 'checked' : '') : value)) {
  ...
  if (animatableElements.length > 0) {
    ...
    function applyStaggerDelay(element, index) {
      ...
      if (siblings.length > 1) {
        ...
        if (siblingIndex > -1) {
          const delay = element.getAttribute('data-delay') || (siblingIndex * 100);
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "נאות שדה משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'dea98df6-eb38-4f05-a9c1-283430fc7462',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'dea98df6-eb38-4f05-a9c1-283430fc7462';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'dea98df6-eb38-4f05-a9c1-283430fc7462';
